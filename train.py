...
if __name__ == "__main__":
    ...
    capnet.initialize(mx.init.Xavier(magnitude=2), ctx=ctx)
    capnet.collect_params().reset_ctx(ctx)
    clsnet = ClsNet(args.batch_size, ctx)
    clsnet.initialize(mx.init.Xavier(magnitude=2), ctx=ctx)
    clsnet.collect_params().reset_ctx(ctx)
    print capnet.collect_params()
    captrainer = Trainer(capnet.collect_params(),'adam', {'learning_rate': 0.001})
    # captrainer = Trainer(capnet.collect_params(), 'sgd', {'learning_rate': 0.0001, 'wd': 5e-4})
    if args.recon:
